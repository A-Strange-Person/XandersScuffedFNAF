...
power = Label(100.0,50,40,fill='white',size=20)
...
mens = Rect(northeast.right+8,northeast.bottom,40,40,fill='gray',border='black',borderWidth=6,align='bottom-left') 
womens = Rect(northeast.right+8,mens.top-8,40,40,fill='gray',border='black',borderWidth=6,align='bottom-left')
...
EastLight = Polygon(east.left+6,office.top,east.right-6,office.top-4,east.right-6,office.centerY+4,east.left+6,office.centerY,fill='darkGray',visible=False)
WestLight = Polygon(west.right-6,office.top,west.left+6,office.top-4,west.left+6,office.centerY+4,west.right-6,office.centerY,fill='darkGray',visible=False)
...
def westLight():
    if WestLight.visible == False:
        ...
        if blueAI.location == 9:blue.visible = True
        else: blue.visible = False
        if redAI.location == 9: red.visible = True
        ...
        if blueAI.location == 9:blue.visible=False
        else: blue.visible = False
        if redAI.location == 9:red.visible = False
        else: red.visible = False
        power.level -= 1
        power.timer = 0
...
def eastLight():
    if EastLight.visible == False:
        ...
        if yellowAI.location == 12:yellow.visible = True
        else: yellow.visible = False
        power.level += 0.5
        power.timer = 0
        power.value -= 1
    else:
        EastLight.visible = False
        if yellowAI.location == 12:yellow.visible=False
        else: yellow.visible = False
        power.level -= 1
        power.timer = 0
...
def moveBlue():
    if blueAI.location == 0: # if blue is at stage
        ...
        else: blueAI.location = 14 #go by player
...
def onStep():
    if power.level == 1:power.rate = 48
    elif power.level == 2:power.rate=24
    elif power.level == 3:power.rate=14
    elif power.level == 4:power.rate=6
    elif power.level == 5:power.rate=3
    elif power.level == 6:power.rate=2
    
    power.timer += 1
    if power.timer == power.rate:
        power.timer = 0
        if power.value >= 1: power.value -= 0.5
    ...
    if redAI.timer == redAI.timerD:
        redAI.timer = 0
        dice = r.randint(1,20)
        if redAI.level >= dice and power.camsOn == False:moveRed()
        else: pass
    
    if blueAI.location == 9 and WestLight.visible == False: blue.visible = False
    elif power.camsOn == True and blueAI.location != 9: blue.visible = True
    if redAI.location == 9 and WestLight.visible == False: red.visible = False
    elif power.camsOn == True and redAI.location != 9: red.visible = True
    if yellowAI.location == 12 and EastLight.visible == False: yellow.visible = False
    elif power.camsOn == True and yellowAI.location != 12: yellow.visible = True
